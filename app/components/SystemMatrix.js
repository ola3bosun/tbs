...
export default function SystemMatrix({ track, activeHover }) {
  ...
  useEffect(() => {
    ...
    if (track.isPlaying) {
      setLogs((prev) => [
        ...prev,
        `[${timestamp}] Spotify API link: active // listening to "${track.title}"`,
      ]);
    } else if (logs.length > 4) {
      setLogs((prev) => [
        ...prev,
        `[${timestamp}] Spotify signal lost // parsing static noise.`,
      ]);
    }
  ...
  useEffect(() => {
    ...
    setLogs((prev) => [
      ...prev,
      `[${timestamp}] Event trigger // ${activeHover}`,
    ]);
  }, [activeHover]);
  ...
  return (
    <div className="hidden md:flex flex-col space-y-6 font-mono text-[10px] text-neutral-500 tracking-wider uppercase h-[220px] justify-between pt-2">
      ...
      <div className="space-y-4">
        <div className="space-y-1">
          ...
          <p className="text-neutral-400">
            Local Time:{" "}
            <span className="text-[#f4efe9] tabular-nums">
              {time || "00:00:00"}
            </span>
          </p>
        </div>
      ...
      <div className="space-y-2 flex-1 flex flex-col min-h-0 justify-end">
        <span className="text-neutral-600 block border-b border-neutral-900 pb-1">
          // Activity Log
        </span>
        <div
          ref={logContainerRef}
          ...
          style={{
            ...
            WebkitMaskImage:
              "linear-gradient(to bottom, transparent 0%, white 20%)",
          }}
        >
          {logs.map((log, idx) => (
            <p
              key={idx}
              className="truncate tracking-wide text-[9px] font-light text-neutral-500 first-of-type:text-[#f4efe9]"
            >
              {log}
            </p>
          ))}
